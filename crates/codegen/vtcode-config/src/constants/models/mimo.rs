pub const DEFAULT_MODEL: &str = MIMO_V2_6_PRO;

pub const MIMO_V2_6_PRO: &str = "mimo-v2.6-pro";
pub const MIMO_V2_6_FLASH: &str = "mimo-v2.6-flash";
pub const MIMO_V2_6_PRO_ULTRASPEED: &str = "mimo-v2.6-pro-ultraspeed";
const MIMO_V2_5_ASR: &str = "mimo-v2.5-asr";
const MIMO_V2_5_TTS: &str = "mimo-v2.5-tts";
const MIMO_V2_5_TTS_VOICECLONE: &str = "mimo-v2.5-tts-voiceclone";
const MIMO_V2_5_TTS_VOICEDESIGN: &str = "mimo-v2.5-tts-voicedesign";
const MIMO_V2_PRO: &str = "mimo-v2-pro";
const MIMO_V2_OMNI: &str = "mimo-v2-omni";
const MIMO_V2_TTS: &str = "mimo-v2-tts";

/// Models available via pay-as-you-go API
pub const PAYG_MODELS: &[&str] = &[MIMO_V2_6_PRO, MIMO_V2_6_FLASH, MIMO_V2_6_PRO_ULTRASPEED];

/// Models available via Token Plan (superset of pay-as-you-go)
pub const TOKEN_PLAN_MODELS: &[&str] = &[
    MIMO_V2_6_PRO,
    MIMO_V2_6_FLASH,
    MIMO_V2_6_PRO_ULTRASPEED,
    MIMO_V2_5_ASR,
    MIMO_V2_5_TTS,
    MIMO_V2_5_TTS_VOICECLONE,
    MIMO_V2_5_TTS_VOICEDESIGN,
    MIMO_V2_PRO,
    MIMO_V2_OMNI,
    MIMO_V2_TTS,
];

/// All supported models (union of both auth methods)
pub(crate) const SUPPORTED_MODELS: &[&str] = TOKEN_PLAN_MODELS;
