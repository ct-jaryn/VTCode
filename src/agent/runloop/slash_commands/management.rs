...
pub(super) fn handle_mcp_command(args: &str, renderer: &mut AnsiRenderer) -> Result<SlashCommandOutcome> {
    ...
    match subcommand.as_str() {
        "status" | "overview" => Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::Overview }),
        "list" | "providers" => Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::ListProviders }),
        "tools" => Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::ListTools }),
        "refresh" | "reload" => Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::RefreshTools }),
        "config" => {
            if tokens.len() > 1 {
                let mode = tokens[1].to_ascii_lowercase();
                match mode.as_str() {
                    "edit" | "--edit" => Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::EditConfig }),
                    "show" | "list" | "status" => {
        ...
        "login" => {
            ...
            Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::Login(tokens[1].clone()) })
        }
        "logout" => {
            if tokens.len() < 2 {
                render_mcp_usage(renderer)?;
        ...
        other if other.starts_with("--") => {
            if other == "--list" {
                return Ok(SlashCommandOutcome::ManageMcp { action: McpCommandAction::ListProviders });
            }
            render_mcp_usage(renderer)?;
            Ok(SlashCommandOutcome::Handled)
        }
...
pub(super) fn handle_local_command(args: &str, renderer: &mut AnsiRenderer) -> Result<SlashCommandOutcome> {
    let trimmed = args.trim();
    if trimmed.is_empty() {
        return Ok(SlashCommandOutcome::ManageLocalServer { action: LocalServerAction::Interactive });
    }
    ...
    let normalize =
        |s: &str| -> Option<String> { LocalProvider::from_key(&s.to_ascii_lowercase()).map(|p| p.key().to_string()) };

    let first = tokens[0].to_ascii_lowercase();
    match first.as_str() {
        "status" => {
            let provider = tokens.get(1).and_then(|s| normalize(s));
            Ok(SlashCommandOutcome::ManageLocalServer { action: LocalServerAction::Status { provider } })
        }
        "start" => {
            let provider = tokens.get(1).and_then(|s| normalize(s));
            Ok(SlashCommandOutcome::ManageLocalServer { action: LocalServerAction::Start { provider } })
        }
        "stop" => {
            let provider = tokens.get(1).and_then(|s| normalize(s));
            Ok(SlashCommandOutcome::ManageLocalServer { action: LocalServerAction::Stop { provider } })
        }
        "configure" | "config" => {
            let provider = tokens.get(1).and_then(|s| normalize(s));
            Ok(SlashCommandOutcome::ManageLocalServer { action: LocalServerAction::Configure { provider } })
        }
            ...
            if let Some(resolved) = LocalProvider::from_key(&first) {
                ...
                if tokens.len() > 1 {
                    ...
                    let action = match action_str.as_str() {
                        ...
                        "configure" | "config" => LocalServerAction::Configure { provider: Some(canonical.clone()) },
                        "troubleshoot" | "diagnose" | "fix" => {
