...
if [[ "$RUN_MSRV" == true ]]; then
	...
	echo -e "${BLUE}Expected MSRV:${NC} 1.98.1 (Rust 2024 Edition)"
	echo ""
	...
	# Verify every toolchain pin agrees on one version. rust-toolchain.toml
	# is the single source of truth; anything else pinned differently means
	# the repo is silently using multiple toolchains.
	echo ""
	echo -e "${BLUE}Checking toolchain pin consistency...${NC}"
	PINNED=$(grep -E '^channel[[:space:]]*=' rust-toolchain.toml | sed 's/.*"\([^"]*\)".*/\1/')
	echo -e "${BLUE}Pinned toolchain:${NC} $PINNED"
	PINS_CONSISTENT=true
	check_pin() {
		local file="$1" actual="$2"
		if [[ "$actual" != "$PINNED" ]]; then
			echo -e "${RED}✗ $file pins $actual, expected $PINNED${NC}"
			PINS_CONSISTENT=false
		fi
	}
	while IFS= read -r cargo_toml; do
		ver=$(grep -o 'rust-version = "[^"]*"' "$cargo_toml" | head -n 1 | sed 's/.*"\([^"]*\)".*/\1/')
		if [[ -n "$ver" ]]; then
			check_pin "$cargo_toml" "$ver"
		fi
	done < <(find . -name 'Cargo.toml' -not -path './target/*' -not -path './.worktrees/*' -not -path './patches/*' -not -path './fuzz/*' -not -path './extensions/*')
	check_pin 'clippy.toml' "$(grep -E '^msrv[[:space:]]*=' clippy.toml | sed 's/.*"\([^"]*\)".*/\1/')"
	check_pin 'Dockerfile.build' "$(grep -E '^FROM rust:' Dockerfile.build | sed 's/.*rust:\([^-]*\).*/\1/')"
	check_pin '.github/workflows/ci.yml (MSRV job)' "$(grep -A18 'MSRV Check' .github/workflows/ci.yml | grep -o 'toolchain: "[^"]*"' | head -n 1 | sed 's/.*"\([^"]*\)".*/\1/')"
	if [[ "$PINS_CONSISTENT" == true ]]; then
		echo -e "${GREEN}✓ All toolchain pins agree on $PINNED${NC}"
	else
		OVERALL_STATUS=1
	fi
